...
OSApp.EquipmentCatalog.types = [
	...
	{ value: "hose", label: "Porous hose (L/hour per meter)" },
...
OSApp.EquipmentCatalog.validate = function( data ) {
	if ( !data || data.version !== 1 || !Array.isArray( data.entries ) ) { throw new Error( "Unsupported equipment catalog." ); }
	var ids = new Set(), names = new Set();
	data.entries.forEach( function( e ) {
		if ( !e || typeof e.id !== "string" || !e.id.trim() || ids.has( e.id ) || typeof e.name !== "string" || !e.name.trim() || names.has( e.name.trim().toLowerCase() ) ) { throw new Error( "Each catalog entry needs a unique ID and name." ); }
		...
		if ( e.type === "dripline" && ( !Number.isFinite( e.spacing ) || e.spacing <= 0 ) ) { throw new Error( "Enter the distance between drippers along the hose in meters, greater than zero." ); }
...
OSApp.EquipmentCatalog.save = function( baseline, data ) {
	if ( JSON.stringify( OSApp.EquipmentCatalog.load() ) !== JSON.stringify( baseline ) ) { throw new Error( "Catalog changed in another window. Reopen before saving." ); }
...
OSApp.EquipmentCatalog.calculate = function( entry, geometry ) {
	...
	function positive( key ) {
		...
		if ( !Number.isFinite( n ) || n <= 0 ) { throw new Error( {
			rows: "Enter hose spacing, or planting-strip width for a single row, in meters greater than zero (for example, 0.5 for 50 cm).",
			count: "Enter the number of emitters watering this area, as a whole number greater than zero.",
			length: "Enter the length of water-releasing hose in meters, greater than zero.",
			area: "Enter the planting area served by these emitters or hose in square meters, greater than zero."
		}[ key ] ); }
		return n;
	}
	var rate = entry.flow;
...
OSApp.EquipmentCatalog.displayPage = function() {
	var page = OSApp.SoilPrograms.page( "equipment-catalog", "Equipment catalog", "#os-options" ), body = page.find( "main" ), data, baseline;
	if ( OSApp.currentSession.controller.options.smode !== 1 ) { body.append( "<p>Select and save Soil water balance to manage the catalog.</p>" ); return; }
	...
	function edit( entry ) {
		...
		fields.spacing = OSApp.SoilPrograms.field( editor, "catalog-spacing", "Spacing between drippers (meters)", e.spacing, "number" );
		...
		if ( entry ) {
			$( "<button id='delete-equipment' class='ui-btn'>Delete equipment</button>" ).appendTo( editor ).on( "click", function() {
				OSApp.UIDom.areYouSure( "Delete this catalog entry?", "Existing programs keep their copied settings.", function() {
	...
	render();
	body.append( "<h2>Catalog backup</h2><p>Copy this JSON to keep a backup or transfer entries to another browser. Import replaces the catalog, without changing programs.</p>" );
	var backup = $( "<textarea id='catalog-json' aria-label='Catalog JSON' rows='8'></textarea>" ).appendTo( body );
	$( "<button id='export-catalog' class='ui-btn'>Export catalog</button>" ).appendTo( body ).on( "click", function() { backup.val( JSON.stringify( data, null, 2 ) ); } );
	$( "<button id='import-catalog' class='ui-btn'>Import catalog</button>" ).appendTo( body ).on( "click", function() {
		var next;
		try { next = OSApp.EquipmentCatalog.validate( JSON.parse( backup.val() ) ); } catch ( e ) { OSApp.Errors.showError( e.message ); return; }
		OSApp.UIDom.areYouSure( "Replace this browser\u2019s equipment catalog?", "Existing programs keep their copied settings.", function() { if ( persist( next ) ) { editor.empty(); render(); } } );
	} );
};
OSApp.EquipmentCatalog.programHelper = function( parent, fields, program ) {
	var entries;
	try { entries = OSApp.EquipmentCatalog.load().entries; } catch ( e ) { parent.append( $( "<p></p>" ).text( e.message ) ); return function() { throw new Error( "Catalog unavailable. Choose Manual entry or repair the catalog." ); }; }
	...
	[ [ "rows", "Hose spacing / single-row width (meters)", "Distance from the center of one length of drip hose to the center of the next parallel length. For example, enter 0.5 for hoses 50 cm apart. This is not the spacing between drippers along a hose. For a single hose along a row of raspberries, enter the width of the planting strip served by that hose instead. For example, a 10-meter hose serving a strip 0.5 meters wide represents 5 square meters: enter 0.5 here. Use the planting-strip width, not just the visible wet spots. For irregular layouts, use emitter count and planting area instead." ],
		[ "count", "Number of emitters", "Count the drippers or small sprinklers supplying the planting area entered below. This calculation assumes they all have the selected flow rating. Count emitters, not meters of supply tubing." ],
		[ "length", "Water-releasing hose length (meters)", "Total length of porous or soaker hose that releases water into this planting area. Do not include plain supply tubing. For example, two 10-meter lengths total 20 meters." ],
		[ "area", "Planting area served (m\u00b2)", "Area of planting served by the emitters or hose entered above. For a rectangular bed, multiply length by width: 5 meters by 2 meters is 10 square meters. Use the same planting area represented by the soil-water balance, not just the small wet spots around drippers." ] ].forEach( function( pair ) {
	...
	function update() {
		...
		try { preview.text( entry ? "Estimated gross application rate: " + OSApp.EquipmentCatalog.calculate( entry, values() ).toFixed( 3 ) + " mm/hour. Efficiency: " + ( entry.efficiency === "" ? "not configured; set an estimate in Equipment catalog maintenance, or choose Manual entry" : entry.efficiency + "% (estimate)" ) : "" ); } catch ( e ) { preview.text( e.message ); }
	}
	function changed() {
		dirty = true; update();
	...
	return function() {
		...
		if ( Number( fields.rate.val() ) !== applied.appliedRate || String( fields.efficiency.val() ) !== String( applied.entry.efficiency ) ) {
			throw new Error( "Apply the equipment values to use catalog calibration, or choose Manual entry to keep your custom values." );
		}
		return snapshot;
	};
};
